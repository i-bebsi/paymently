...
- **`exception/GlobalExceptionHandler`** — `@RestControllerAdvice`. Maps `ResourceAccessException` → 504 Gateway Timeout. Maps `RestClientResponseException` → upstream status code + body (forwarded as-is). Maps uncaught `RuntimeException` → 500. All return structured error body with timestamp, status, error, message, and path.
