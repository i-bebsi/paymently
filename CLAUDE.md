...
```
POST /api/v1/bill/inquiry  or  GET /api/v1/bill/health
  → BillController
    → PaymentMiddlewareService
      → TokenService.getAccessToken() (cached→upstream /v1.0/access-token/b2b)
      → RestTemplate (Apache HttpClient 5) → upstream /v2/bill/inquiry or /v2/bill/healthz
```

### Layers

- **`controller/BillController`** — Two endpoints:
  - `POST /api/v1/bill/inquiry` — proxies bill inquiry to upstream
  - `GET /api/v1/bill/health` — proxies health check to upstream `/v2/bill/healthz`
...
- **`service/PaymentMiddlewareService`** — Calls `tokenService.getAccessToken()` to get a bearer token, then builds common auth headers (Bearer, CHANNEL-ID, X-EXTERNAL-ID, X-PARTNER-ID, X-SIGNATURE, X-TIMESTAMP) via `buildCommonHeaders()`. Handles both `inquiryBill()` (POST) and `healthz()` (GET). On timeout/connection errors, wraps `ResourceAccessException` in a `RuntimeException` with `"Connect to <host>:443 failed: ..."`.
...
- `payment.api.base-url` / `access-token-path` / `inquiry-path` / `payment-path` / `healthz-path` — upstream API endpoint parts.
